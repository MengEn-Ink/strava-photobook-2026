...
封面只使用已经下载到画册内的照片，不会额外请求 API。若当年没有合格照片，生成器会自动使用年度路线海报，不会留下空图片框。竖版照片满屏显示且不叠加可见文案；横版照片完整放在独立图片区，活动标题、日期、里程、爬升和照片说明放在图片外的信息区。点击画册左半区域向前翻页，点击右半区域向后翻页；拖动、方向键和月份时间轴仍然可用。
